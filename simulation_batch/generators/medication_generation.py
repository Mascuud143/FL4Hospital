...
# - Uses diagnosis schedules and admission windows
# - Writes medication rows - MedicationGenerator
...
def _build_rows_for_chunk(chunk: list[dict], diagnoses: Dict, seed: int) -> list[dict]:
    ...
    for admission in chunk:
        ...
        stay_start = as_utc(admission["stay_start"])
        stay_end = as_utc(admission["stay_end"])
            ...
            for drug_name, hours in med_schedule.items():
                for hour in hours:
                    ...
                    if ts < stay_start or ts >= stay_end:
                        continue
                    rows.append({"patient_id": admission["patient_id"], "medication_time": ts, "drug_name": drug_name, "route": "oral", "dose": "1 dose", "status": "taken"})
            day_cursor += timedelta(days=1)
    return rows


class MedicationGenerator:
    def __init__(self, *, seed: int = 42, diagnoses: Dict, workers: int | None = None, chunk_size: int = 50000, write_batch_size: int = 50000):
        self.seed = seed
        ...
        with session_scope() as session:
            ...
            for adm in admissions:
                ...
                if stay_start >= stay_end:
                    continue
                work_items.append({"patient_id": adm.patient_id, "diagnosis": diagnosis, "stay_start": stay_start, "stay_end": stay_end})
            chunks = list(_chunked(work_items, self.chunk_size))
            if not chunks:
                return 0
            if self.workers <= 1 or len(chunks) == 1:
                generated = [_build_rows_for_chunk(chunk, self.diagnoses, self.seed + idx) for idx, chunk in enumerate(chunks)]
            else:
                with ProcessPoolExecutor(max_workers=self.workers) as executor:
                    generated = list(executor.map(_build_rows_for_chunk, chunks, [self.diagnoses] * len(chunks), [self.seed + idx for idx in range(len(chunks))]))
            inserted = 0
