...
def _build_rows_for_chunk(chunk: list[dict], seed: int) -> list[dict]:
    ...
    for item in chunk:
        ...
        stay_start = _as_utc(item["stay_start"])
        stay_end = _as_utc(item["stay_end"])
        medication_times = [_as_utc(ts) for ts in item.get("medication_times", [])]
                ...
                rows.append(
                    {
                        ...
                        "visit_time": ts,
                        "body_temperature": _body_temperature_for_symptom(rng, symptom),
                        "blood_pressure": _blood_pressure_for_symptom(rng, symptom),
                        "symptoms": symptom,
                    }
                )
    ...
    def generate_for_horizon(
        ...
        with session_scope() as session:
            ...
            for med in medications:
                meds_by_patient[int(med.patient_id)].append(_as_utc(med.medication_time))
            ...
            for adm in admissions:
                ...
                medication_times = [
                    ts for ts in meds_by_patient.get(patient_id, [])
                    if stay_start <= ts < stay_end
                ]
                work_items.append(
                    {
                        "patient_id": patient_id,
                        "diagnosis": adm.current_diagnosis or "",
            ...
            if self.workers <= 1 or len(chunks) == 1:
                generated_iter = (
                    _build_rows_for_chunk(chunk, self.seed + idx)
                    for idx, chunk in enumerate(chunks)
                )
            else:
                executor = ProcessPoolExecutor(max_workers=self.workers)
                generated_iter = executor.map(
