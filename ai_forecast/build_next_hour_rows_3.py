...
DEFAULT_OUT_DIR = "outputs_next_hour_3"
...
def candidate_times_for_assignment(
    ...
    change_event_times: list[datetime],
    before_minutes: int,
    after_minutes: int,
    sample_minutes: int,
) -> list[datetime]:
    ...
    candidates = {start}
    for ts in times_in_window(change_event_times, start - after, end + before):
...
def build_rows(
    ...
    with open(out_path, "w", encoding="utf-8", newline="") as f:
        ...
        for assignment in assignments:
            ...
            change_event_times = change_event_times_by_room.get(room_id, [])
    ...
    print("build_next_hour_rows_3.py complete")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build next-hour rows around y_change events.")
    ...
    parser.add_argument("--before-minutes", type=int, default=60, help="Minutes before each y_change event to sample.")
    parser.add_argument("--after-minutes", type=int, default=60, help="Minutes after each y_change event to sample.")
