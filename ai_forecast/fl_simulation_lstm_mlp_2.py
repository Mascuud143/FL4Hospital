...
from fl_client_lstm_mlp_2 import (
    RoomHybridClientV2,
    TARGET_COLUMNS,
    build_hybrid_arrays,
    get_input_dim,
...
from fl_server_lstm_mlp_2 import make_initial_parameters


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Flower simulation mode for next-hour hybrid MLP+LSTM training v2.")
    ...
    parser.add_argument("--weights-out-dir", default="ai/fl_weights_sim_lstm_mlp_2", help="Directory to write global weights per round")
    ...
    parser.add_argument("--server-address", default="127.0.0.1:8094", help="Used for threaded fallback mode")
...
def run_threaded_fallback(args: argparse.Namespace, strategy: TrackingFedAvg, room_ids: list[str], room_data: dict[str, tuple], input_dim: int) -> None:
    ...
    server_thread = threading.Thread(target=server_target, name="fl_lstm_mlp_2_server")
    server_thread.start()
    time.sleep(args.server_start_wait)

    client_threads: list[threading.Thread] = []
    for rid in room_ids:
        thread = threading.Thread(target=client_target, args=(rid,), name=f"fl_lstm_mlp_2_client_{rid}")
        thread.start()
...
def main() -> None:
    ...
    if not room_ids:
        raise RuntimeError("No room sequence training data loaded for hybrid LSTM+MLP simulation v2.")
    ...
    print("fl_simulation_lstm_mlp_2.py starting")
