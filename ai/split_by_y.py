...
DEFAULT_OUTPUT_DIR = "splits_by_y"
...
def filter_rows_one_hour_before_positive_events(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for client_rows in grouped.values():
        ...
        positive_times = [
            ...
            if (row.get("y_event") or "").strip() == "1"
        ]
        if not positive_times:
            continue

        for row in client_rows:
            row_time = parse_ts(row.get("t"))
            if any(positive_time - lookback <= row_time <= positive_time for positive_time in positive_times):
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Split rows into train/test, keeping only rows within one hour before positive y_event rows."
    )
...
def main() -> None:
    ...
    filtered_model_a_rows = filter_rows_one_hour_before_positive_events(model_a_rows)

    a_train, a_test, a_train_counts, a_test_counts = split_rows_by_client(
        filtered_model_a_rows, args.train_ratio, args.min_train_rows
    )
    filtered_model_b_rows = model_b_rows

    b_train, b_test, b_train_counts, b_test_counts = split_rows_by_client(
        filtered_model_b_rows, args.train_ratio, args.min_train_rows
    )
    ...
    print("split_by_y.py complete")
    print(f"model_a_total={len(model_a_rows)} filtered={len(filtered_model_a_rows)} train={len(a_train)} test={len(a_test)}")
    print(f"model_b_total={len(model_b_rows)} filtered={len(filtered_model_b_rows)} train={len(b_train)} test={len(b_test)}")
