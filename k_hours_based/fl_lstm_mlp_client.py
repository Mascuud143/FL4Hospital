...
from runtime_defaults import DEFAULT_AIRFLOW_THRESHOLD, DEFAULT_CHANGE_THRESHOLD
...
class RoomHybridClient(fl.client.NumPyClient):
    def __init__(
        ...
        self.airflow_threshold = DEFAULT_AIRFLOW_THRESHOLD
    ...
    def _evaluate_arrays(
        ...
        self.model.eval()
        with torch.no_grad():
            y_pred = self.model.target_lstm(
                torch.tensor(x_eval_seq, dtype=torch.float32, device=self.device)
            ).cpu().numpy()
            change_logits = self.model.change_mlp(
                torch.tensor(x_eval_flat, dtype=torch.float32, device=self.device)
            ).cpu().numpy()
        ...
        return summarize_next_hour_predictions(
            ...
            y_eval,
            y_pred,
            change_eval.astype(np.int64),
            change_pred,
            airflow_pred=airflow_pred,
            extra_metrics={
...
def main() -> None:
    ...
    x_train_seq, x_train_flat, y_train, current_train, change_train = build_hybrid_arrays(train_df, args.sequence_length)
